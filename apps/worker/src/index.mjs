...
const required = (name) => {
  ...
  if (!value) throw new Error(`${name} ausente no worker.`);
  return value;
};
...
const POLL_INTERVAL_MS = Number(env("WORKER_POLL_INTERVAL_MS", "5000"));
...
async function log(row) {
  try {
    await insert("system_logs", {
      module: "external-ai-worker",
      type: "worker",
      status: row.status || "info",
      friendly_message: row.message || "Evento do worker externo.",
...
async function processContent(job, post) {
  ...
  const payload = await openAiJson([
    { role: "system", content: "Voce e um social media senior premium para incorporadoras. Responda somente JSON valido." },
    {
      ...
      content: [
        ...
        `Post: ${text(post, 2200)}`,
        `Contexto da marca: ${text(ctx, 3200)}`,
  ...
  await insert("post_versions", {
    ...
    post_id: post.id,
    version_label: `WORKER-COPY-${Date.now()}`,
...
async function processImage(job, post) {
  ...
  const prompt = [
    ...
    "Arquitetura brasileira contemporanea, alto padrao, fundo claro/off-white/areia, luz natural, sofisticacao limpa.",
  ...
  const asset = await insert("media_assets", {
    ...
    status: "ativo",
    tags: ["ia", "myinc", "worker"],
    origin: `external-worker:${model}`,
    ...
    is_final: true,
    notes: prompt,
    metadata: { image_model: model, source: "external-worker" },
  });

  const updated = await update("posts", `id=eq.${post.id}`, {
    media_url: publicUrl,
  ...
  await insert("post_versions", {
    brand_id: post.brand_id,
    post_id: post.id,
    version_label: `WORKER-IMAGE-${Date.now()}`,
    caption: post.caption,
    image_prompt: prompt,
    media_url: publicUrl,
    is_current: true,
    output_json: { media_url: publicUrl, asset_id: asset?.id, image_model: model },
  });
...
async function complete(job, result) {
  await update("generation_jobs", `id=eq.${job.id}`, {
    status: "completed",
    progress: 100,
    result,
    output_json: result,
...
async function fail(job, error) {
  ...
  if (attempt >= max) {
    ...
    if (job.post_id) await update("posts", `id=eq.${job.post_id}`, { status: "erro_ia", error_message: "Falha definitiva no worker externo.", technical_detail: detail, updated_at: new Date().toISOString() });
  } else {
    await update("generation_jobs", `id=eq.${job.id}`, {
      status: "queued",
      progress: 0,
...
async function processJob(job) {
  ...
  else if (type === "video") throw new Error("Video ainda nao habilitado neste worker. Use ENABLE_VIDEO_WORKER depois.");
  else throw new Error(`Tipo de job nao suportado: ${type}`);

  await complete(job, result);
  await log({ status: "sucesso", message: "Job processado fora do Supabase Edge.", detail: `job=${job.id}; type=${type}`, brand_id: post.brand_id, post_id: post.id });
}
...
async function tick() {
  ...
  try {
    ...
    await log({ status: "erro", message: "Falha no worker externo.", detail: `job=${job.id}; error=${error?.message || String(error)}`, post_id: job.post_id });
  }
  return true;
}

console.log(`[${WORKER_ID}] MYINC AI Worker iniciado.`);
while (true) {
  try {
    const processed = await tick();
    if (!processed) await sleep(POLL_INTERVAL_MS);
  } catch (error) {
    console.error(`[${WORKER_ID}] erro`, error);
