...
function isArchivedOrDeleted(post: Pick<PostRow, "status" | "archived_at" | "deleted_at">) {
  ...
  return Boolean(
    post.archived_at ||
    post.deleted_at ||
    ["arquivado", "excluido", "excluído", "deleted", "deletado"].includes(status),
  );
}
...
async function getActivePost(token: string, postId: string) {
  ...
  if (isArchivedOrDeleted(post))
    throw new Error("Post arquivado/excluído não pode entrar em produção.");
  return post;
}
...
async function queueMediaBatch(
  token: string,
...
export function generatePostContent(token: string, postId: string, instruction?: string) {
  return callEdgeFunction<{ ok: true; post: PostRow; message?: string }>(
    "generate-post-content-safe",
    token,
    { postId, instruction },
  );
}
...
export async function reviewPostQuality(token: string, postId: string) {
  const post = await postRepository.update(token, postId, {
    ...
    quality_review: {
      ...
      notes:
        "Revisão registrada pelo Estúdio Criativo. Use revisão humana antes de publicar campanhas reais.",
    },
...
export function renderPostTemplate(token: string, postId: string) {
  return callEdgeFunction<{ ok: true; post: PostRow; mediaUrl?: string; message?: string }>(
    "render-template",
    token,
    { postId },
  );
...
export function createProductionBatch(
  token: string,
  payload: { brandId: string; postIds: string[]; instruction?: string; includeContent?: boolean },
) {
  return callEdgeFunction<{
    ok: true;
    batchId: string;
    ...
    skipped?: unknown[];
    jobs?: unknown[];
  }>("process-production-queue", token, payload);
}
